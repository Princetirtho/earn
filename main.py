...
# ---------------- CONFIGURATION ----------------
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8925659690:AAGNgo5JF_j1tI-61hbjb6AgPHepeUunyN0")

# Multiple Admin IDs
ADMIN_IDS = [8212595643, 8235339975]

# Conversation States
USERNAME, PASSWORD, TXN_ID, WITHDRAW_PHONE, WITHDRAW_AMOUNT, SUPPORT_MSG, SEARCH_USER, USER_DETAILS, ACTIVATION_METHOD, WITHDRAW_METHOD, ADMIN_SEARCH_TXN, DATABASE_RESTORE = range(12)
...
def get_all_users():
    conn = sqlite3.connect('share2pay.db')
    cursor = conn.cursor()
    cursor.execute("SELECT user_id, username, is_active, balance, total_referrals, total_withdraw, join_date, is_blocked FROM users ORDER BY join_date DESC")
...
def get_user_details(user_id):
    ...
    cursor.execute("""
        SELECT u.user_id, u.username, u.is_active, u.balance, u.total_referrals, u.total_withdraw, u.join_date,
               r.username as referred_by_username, u.referral_code, u.is_blocked
        FROM users u
...
def search_users(query):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT user_id, username, is_active, balance, total_referrals, total_withdraw, join_date, is_blocked
        FROM users 
...
def get_top_referrers(limit=10):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT user_id, username, total_referrals, balance, is_active, join_date
        FROM users 
...
def restore_database_from_csv(csv_content):
    """Restore database from CSV content"""
    conn = sqlite3.connect('share2pay.db')
    cursor = conn.cursor()
        ...
        for row in reader:
            ...
            if row[0].startswith('=== ') and row[0].endswith(' ==='):
                ...
                if current_table and rows:
                    ...
                    cursor.execute(f"DELETE FROM {current_table}")
                    placeholders = ','.join(['?' for _ in columns])
                    for data_row in rows:
                        try:
                            cursor.execute(f"INSERT INTO {current_table} ({','.join(columns)}) VALUES ({placeholders})", data_row)
                        except Exception as e:
                            logging.error(f"Error inserting row into {current_table}: {e}")
                current_table = row[0].replace('=== ', '').replace(' ===', '').lower()
                columns = []
                rows = []
...
def get_admin_reply_keyboard():
    keyboard = [
        ...
        [KeyboardButton("🏆 টপ রেফারার"), KeyboardButton("🔍 ট্রানজেকশন সার্চ")],
        [KeyboardButton("📥 CSV ডাউনলোড"), KeyboardButton("📤 ডাটাবেস রিস্টোর")],
...
def get_user_action_keyboard(user_id, is_blocked):
    keyboard = []
    if is_blocked:
        keyboard.append([InlineKeyboardButton("🔓 আনব্লক করুন", callback_data=f"unblock_{user_id}")])
    else:
        keyboard.append([InlineKeyboardButton("🔒 ব্লক করুন", callback_data=f"block_{user_id}")])
    keyboard.append([InlineKeyboardButton("🗑️ ইউজার ডিলিট করুন", callback_data=f"delete_{user_id}")])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user[10] == 1:
        await update.message.reply_text(
            ...
            "━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "আপনার অ্যাকাউন্ট অ্যাডমিন দ্বারা ব্লক করা হয়েছে।\n"
...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "❓ কিভাবে করবেন?":
        ...
        if user_details:
            ...
            block_status = "🚫 ব্লক করা" if user_details[9] == 1 else "✅ সক্রিয়"
...
async def handle_admin_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "👥 ইউজার লিস্ট":
        ...
        conn = sqlite3.connect('share2pay.db')
        cursor = conn.cursor()
        cursor.execute("SELECT id, user_id, username, txn_id, phone_number, method, created_at FROM transactions WHERE status='pending' ORDER BY created_at ASC")
        rows = cursor.fetchall()
        conn.close()
        if not rows:
        ...
        conn = sqlite3.connect('share2pay.db')
        cursor = conn.cursor()
        cursor.execute("SELECT id, user_id, username, amount, phone_number, method, created_at FROM pending_withdraws WHERE status='pending' ORDER BY created_at ASC")
        rows = cursor.fetchall()
        conn.close()
        if not rows:
    ...
    elif text == "🏆 টপ রেফারার":
        top_users = get_top_referrers(10)
        if not top_users:
            await update.message.reply_text("📭 **কোনো রেফারার নেই!**")
            return
        msg = "🏆 **টপ ১০ রেফারার**\n━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        for i, user in enumerate(top_users, 1):
            status = "✅" if user[3] == 1 else "❌"
            join_date = datetime.strptime(user[5], '%Y-%m-%d %H:%M:%S.%f').strftime('%d %b, %Y')
    ...
    elif text == "🔍 ট্রানজেকশন সার্চ":
        await update.message.reply_text(
            "🔍 **ট্রানজেকশন সার্চ**\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "📝 ট্রানজেকশন ID অথবা ফোন নম্বর লিখুন:\n"
            "📌 উদাহরণ: `ABC123XYZ` অথবা `01712345678`\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━"
        )
        return ADMIN_SEARCH_TXN
    ...
    elif text == "📤 ডাটাবেস রিস্টোর":
        await update.message.reply_text(
            "📤 **ডাটাবেস রিস্টোর**\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "⚠️ **সাবধান!** এটি সমস্ত ডেটা রিপ্লেস করবে!\n\n"
            "📝 পূর্বে ডাউনলোড করা CSV ফাইল পাঠান:\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━"
        )
...
async def user_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    block_status = "🔒 ব্লক করা" if user[9] == 1 else "✅ সক্রিয়"
    ...
    action_btn = get_user_action_keyboard(user[0], user[9])
    await update.message.reply_text(msg, reply_markup=action_btn, parse_mode='Markdown')
    return ConversationHandler.END

# ---------------- ADMIN SEARCH TRANSACTION ----------------
async def admin_search_txn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.message.text.strip()
    
    conn = sqlite3.connect('share2pay.db')
    cursor = conn.cursor()
    
    # Search in transactions by txn_id or phone_number
    cursor.execute("""
        SELECT id, user_id, username, type, amount, status, txn_id, phone_number, method, created_at 
        FROM transactions 
        WHERE txn_id LIKE ? OR phone_number LIKE ?
        ORDER BY created_at DESC
        LIMIT 20
    """, (f'%{query}%', f'%{query}%'))
    transactions = cursor.fetchall()
    ...
    if not transactions:
        await update.message.reply_text(
            f"❌ **কোনো ট্রানজেকশন পাওয়া যায়নি!**\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🔍 '{query}' এর সাথে মেলে এমন কোনো ট্রানজেকশন নেই।\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━"
        )
        return ConversationHandler.END
    ...
    for i, txn in enumerate(transactions, 1):
        ...
        method_emoji = "📱" if "বিকাশ" in txn[8] else "💳"
        
        # If it's pending, add approve/reject buttons
        if txn[5] == "pending":
            btn = InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ অ্যাপ্রুভ", callback_data=f"app_pay_{txn[0]}_{txn[1]}"),
                 InlineKeyboardButton("❌ রিজেক্ট", callback_data=f"rej_pay_{txn[0]}_{txn[1]}")]
            ])
            await update.message.reply_text(
                f"📥 **ট্রানজেকশন #{i}**\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
                f"🆔 আইডি: `{txn[0]}`\n"
                f"👤 ইউজার: {txn[2]} (ID: `{txn[1]}`)\n"
                f"📊 টাইপ: {txn_type}\n"
                f"{method_emoji} মেথড: {txn[8]}\n"
                f"🔑 TxnID: `{txn[6] or 'N/A'}`\n"
                f"📱 ফোন: `{txn[7] or 'N/A'}`\n"
                f"💰 পরিমাণ: {txn[4]} টাকা\n"
...
# ---------------- DATABASE RESTORE ----------------
async def database_restore(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "⚠️ **সাবধান!**\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "এই কাজটি সম্পূর্ণ ডাটাবেস রিপ্লেস করবে।\n"
        "আপনি কি নিশ্চিত? (হ্যাঁ/না)\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━"
    )
    context.user_data['restore_file'] = document
    return DATABASE_RESTORE

async def database_restore_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    document = context.user_data.get('restore_file')
    if not document:
        await update.message.reply_text(
            "❌ **ফাইল পাওয়া যায়নি!**\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    try:
        ...
        if restore_database_from_csv(csv_text):
            await update.message.reply_text(
                "✅ **ডাটাবেস রিস্টোর সফল!** 🎉\n\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━\n"
                "📌 সমস্ত ডেটা সফলভাবে আপডেট করা হয়েছে।\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━"
            )
        else:
            await update.message.reply_text(
                "❌ **ডাটাবেস রিস্টোর ব্যর্থ!**\n\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━\n"
                "📌 ফাইলটি সঠিক ফরম্যাটে নেই।\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━"
            )
    except Exception as e:
...
async def handle_user_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "block":
        ...
        try:
            await context.bot.send_message(
                target_user_id,
                "🔓 **আপনি আনব্লক করা হয়েছে!**\n\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━\n"
                "আপনি আবার আমাদের সেবা পেতে পারেন।\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━"
            )
        except Exception:
            pass
        ...
        confirm_btn = InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ হ্যাঁ, ডিলিট করুন", callback_data=f"confirm_delete_{target_user_id}"),
             InlineKeyboardButton("❌ বাতিল করুন", callback_data=f"cancel_delete_{target_user_id}")]
        ])
        
        await query.edit_message_text(
            f"⚠️ **ইউজার ডিলিট কনফার্মেশন**\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🆔 ইউজার আইডি: `{target_user_id}`\n\n"
            f"আপনি কি এই ইউজারটি সম্পূর্ণ ডিলিট করতে চান?\n"
            f"📌 সব ডেটা (ট্রানজেকশন সহ) মুছে যাবে!\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━",
        ...
        cursor.execute("DELETE FROM pending_withdraws WHERE user_id=?", (target_user_id,))
        
        conn.commit()
        conn.close()
        
        await query.edit_message_text(
            f"🗑️ **ইউজার ডিলিট করা হয়েছে**\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🆔 ইউজার আইডি: `{target_user_id}`\n"
            f"✅ সমস্ত ডেটা মুছে ফেলা হয়েছে।\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━"
        )
    
    elif action == "cancel_delete":
        await query.edit_message_text(
            f"❌ **ডিলিট প্রক্রিয়া বাতিল করা হয়েছে**\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🆔 ইউজার আইডি: `{target_user_id}`\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━"
        )
...
def main():
    ...
    # Admin Search Transaction
    admin_txn_search_handler = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex('^🔍 ট্রানজেকশন সার্চ$'), handle_admin_messages)],
        states={
            ADMIN_SEARCH_TXN: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_search_txn)],
        },
        fallbacks=[CommandHandler('cancel', cancel)]
    )

    # Database Restore
    db_restore_handler = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex('^📤 ডাটাবেস রিস্টোর$'), handle_admin_messages)],
        states={
            DATABASE_RESTORE: [
                MessageHandler(filters.Document.ALL, database_restore),
                MessageHandler(filters.TEXT & ~filters.COMMAND, database_restore_confirm)
            ],
        },
        fallbacks=[CommandHandler('cancel', cancel)]
    )
    ...
    app.add_handler(admin_txn_search_handler)
    app.add_handler(db_restore_handler)
    ...
    app.add_handler(CallbackQueryHandler(handle_user_action, pattern='^(block|unblock|delete|confirm_delete|cancel_delete)_'))
